...
class Test(TestCase):
    def test_flow_view_decorator(self):
        ...
        task = Task.objects.create(
            process=process, flow_task=TaskTestFlow.task)
    ...
    def test_view_permissins_calc(self):
        ...
        flow_task = flow.View(
            lambda request: None).Assign(
            lambda p: "TEST USER")
        self.assertEqual(flow_task.calc_owner(task), "TEST USER")

        flow_task = flow.View(lambda request: None).Assign(
            {'username': 'test'})
        ...
        flow_task = flow.View(
            lambda request: None).Permission(
            lambda p: "TEST PERM")
        self.assertEqual(flow_task.calc_owner_permission(task), "TEST PERM")
    ...
    def test_view_can_assign(self):
        ...
        permission = Permission.objects.create(
            ...
            codename='existing_perm')
        user.user_permissions.add(permission)
        process = Process.objects.create(flow_class=TaskTestFlow)

        flow_task = flow.View(
            lambda request: None).Permission('auth.existing_perm')
        task = Task.objects.create(
            process=process,
            flow_task=TaskTestFlow.task,
            owner_permission='auth.existing_perm')
        self.assertTrue(flow_task.can_assign(user, task))

        flow_task = flow.View(
            lambda request: None).Permission('auth.unknown_perm')
        task = Task.objects.create(
            process=process,
            flow_task=TaskTestFlow.task,
