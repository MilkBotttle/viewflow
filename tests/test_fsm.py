...
class Test(unittest.TestCase):
    ...
    def test_subchild_transitions(self):
        ...
        self.assertEqual([SubChild.prepare],
                         SubChild.state.get_available_transitions(sub_child))
        sub_child.prepare()
        self.assertEqual('prepared', sub_child.state)

        self.assertEqual([SubChild.start],
                         SubChild.state.get_available_transitions(sub_child))
        sub_child.start()
        self.assertEqual('started', sub_child.state)

        self.assertEqual([SubChild.done],
                         SubChild.state.get_available_transitions(sub_child))
        sub_child.done()
    ...
    def test_subchild_withmixin_transitions(self):
        ...
        self.assertEqual([SubChildWithMixin.prepare],
                         SubChildWithMixin.state.get_available_transitions(sub_child))
        sub_child.prepare()
        self.assertEqual('prepared', sub_child.state)
